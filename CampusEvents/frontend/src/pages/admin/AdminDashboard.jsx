    ...
    const fetchDashboardData = async () => {
      try {
        ...
        const response = await axios.get(
          `${import.meta.env.VITE_API_URL}/api/events/admin/stats`,
          {
  ...
  return (
    <div className="min-h-screen pt-28 pb-12 px-4 md:px-6 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-10">
        ...
        <StatCard
          icon={<QrCode />}
          title="Attendence working on it "
          value="0"
          color="orange-400"
        />
      </div>
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <div className="glass-panel-Rounded p-6 border-l-4 border-l-brand-accent">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div
                ...
                className="p-4 bg-white/5 hover:bg-white/10 border border-white/10 rounded-xl transition-colors flex items-center gap-4 group"
              >
                ...
                <div>
                  ...
                  <p className="text-xs text-text-muted">
                    Group students for Events
                  </p>
                  <p className="text-lg text-text-muted">working on it</p>
                </div>
              </div>

              <div className="p-4 bg-white/5 hover:bg-white/10 border border-white/10 rounded-xl transition-colors flex items-center gap-4 group">
          ...
          <div className="glass-panel-Rounded p-6">
            <div className="flex justify-between items-center mb-6">
              ...
              <span className="text-brand-accent text-sm cursor-pointer hover:underline">
                View All
              </span>
            </div>

            <div className="overflow-x-auto">
              <table className="w-full justify-center text-left border-collapse">
                <thead>
                  <tr className="text-text-muted border-b border-gray-700 ">
                    <th className="pb-3 font-medium text-gradient">
                      Student Name
                    </th>
                    <th className="pb-3 font-medium text-gradient">Branch</th>
                    <th className="pb-3 font-medium text-gradient">
                      Team Name
                    </th>
                    <th className="pb-3 font-medium text-gradient ">Event</th>
                  </tr>
                </thead>
                      ...
                      >
                        <td className="py-4 text-lg text-white font-medium">
                          ...
                          <div className="text-xs text-gray-500 mt-1">
                            {reg.mobile || "No mobile provided"}
                          </div>
                        </td>
                        <td className="py-4 text-xl text-gray-300">
                          {reg?.branch || ""}
                        </td>
                        <td className="py-4 text-md text-gray-300">
                          {reg?.teamName || ""}
                        </td>
            ...
            <div className="space-y-4">
              <div className="p-4 border border-brand-accent/30 bg-brand-accent/5 rounded-xl relative overflow-hidden">
                ...
                <div
                  ...
                  className="w-full py-2 bg-glass-base border border-gray-600 hover:border-brand-accent text-white text-sm rounded-lg transition-colors flex items-center justify-center gap-2"
                >
...
function StatCard({ icon, title, value, color }) {
  const colorMap = {
    ...
    "brand-primary":
      "text-brand-primary bg-brand-primary/20 border-brand-primary",
  ...
  return (
    <div className="glass-panel p-6 rounded-2xl flex items-center gap-4 hover:scale-105 transition-transform cursor-pointer">
      <div
        className={`w-14 h-14 rounded-full flex items-center justify-center shrink-0 border border-opacity-30 ${colorMap[color]}`}
      >
