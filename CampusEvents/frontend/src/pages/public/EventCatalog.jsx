...
const EventCatalog = () => {
  ...
  return (
    <div className="min-h-screen pt-24 pb-12 px-6 max-w-7xl mx-auto">
      {/* Page Header & Search */}
      <div className="mb-12">
        <h1 className="text-4xl font-bold mb-6 text-gradient inline-block">
          Event Catalog
        </h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {events.map((event) => (
            <Link
              to={`/events/${event._id}`}
              key={event._id}
              className="glass-panel-interactive p-4 flex flex-col h-full group"
            >
              <div className="w-full h-40 bg-gray-800 rounded-lg mb-4 overflow-hidden relative">
                ...
                <div
                  className={`absolute top-2 right-2 text-white text-[10px] font-bold px-2 py-1 rounded uppercase backdrop-blur-sm shadow-md ${
              ...
              <span className="text-brand-accent text-xs font-bold mb-1 uppercase tracking-wider">
                {event.category}
              </span>
              ...
              <div className="text-xs text-gray-400 mt-auto space-y-2 pt-3 border-t border-gray-700/50">
                ...
                <p className="truncate flex items-center gap-2">
                  <MapPin size={20} className="text-cyan-100"/> {event.location}
                </p>
              </div>
            </Link>
          ))}
