    ...
    <>
      ...
      >
        ...
        <div className="p-6 border-t border-white/5">
          <button 
            onClick={onLogout}
            className="w-full flex items-center justify-center gap-2 py-3 rounded-xl bg-red-500/10 text-red-400 hover:bg-red-500 hover:text-white border border-red-500/20 hover:border-red-500 hover:shadow-[0_0_15px_rgba(239,68,68,0.4)] transition-all duration-300 font-semibold"
          >
            <LogOut size={20} />
            Logout
          </button>
...
function SidebarLink({ icon, text, badge }) {
  return (
    <div className=" glass-panel-interactive flex items-center justify-between p-3 rounded-xl hover:bg-glass-hover text-text-muted hover:text-white transition-all duration-300 cursor-pointer border border-transparent hover:border-glass-border">
      ...
      {/* If a badge exists (like "2" active tickets), display it */}
      {badge && (
